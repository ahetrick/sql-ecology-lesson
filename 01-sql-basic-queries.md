...
When we built the table, we gave the 'year' column a data type of INT (or integer).
But we could have given it a data type of VARCHAR (or TEXT). Because we are working
with 'year' as an INT, we can just type in 2000. If we had used the VARCHAR or TEXT
data type for 'year', the `WHERE` clause would be `year >= '2000'`. We can use more sophisticated conditions by combining tests
